...
from lumopt.utilities.tensorboard import Board
...
class Optimization(SuperOptimization):
    ...
    def __init__(self, base_script, wavelengths, fom, geometry, optimizer, use_var_fdtd = False, hide_fdtd_cad = False, use_deps = True, plot_history = True, store_all_simulations = True):
        ...
        self.board = None # <-- Initialize later, when we know how many parameters there are 
        self.fomHist = []
        self.paramsHist = []
    ...
    def init_plotter(self):
        if self.plotter is None:
            self.plotter = Plotter(movie = False, plot_history = self.plot_history)
        return self.plotter

    def init_board(self, folder=''):
        if self.board is None:
            self.board = Board(folder)
        return self.board

    def run(self):
        self.initialize()

        # self.init_plotter() # do not use plotter. Instead, use a web page to show data 
        # interactively
        print('working dir = {}'.format(self.workingDir))
        self.init_board(folder = self.workingDir)
    ...
    def initialize(self):
        ...
        def plotting_tensobard_function():
            """
            Send
            - current iteration
            - figure of merit
            ...
            - gradients
            to TensorBoard to plot them.

            Also, save the current parameters to a file and write the current iteration and figure 
            of merit to a file.
            """
            self.board.log_data(self.optimizer.iteration,
        ...
        self.optimizer.initialize(start_params = start_params, callable_fom = callable_fom, callable_jac = callable_jac, bounds = bounds, plotting_function = plotting_tensobard_function)
